...
import { loadingCityAtom, placeAtom, locationAtom } from "../store";
import { useAtom } from "jotai";
import { getLocations,getForecastByLatLong } from "../services/weather";
...
export default function Navbar() {
  ...
  function handleCurrentLocation() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(async (postiion) => {
        ...
        try {
          setLoadingCity(true);
          const response = await getForecastByLatLong(latitude, longitude);
          setTimeout(() => {
            setLoadingCity(false);
  ...
  return (
    <>
      ...
      <section className="flex max-w-7xl px-3 md:hidden justify-center pt-5">
